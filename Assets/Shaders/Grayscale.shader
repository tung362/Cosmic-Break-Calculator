    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                if (_UseGrayscale)
                {
                    if (_SmoothGrayscale) return lerp(start, dot(mainTex.rgb, float3(0.3, 0.59, 0.11)) * i.color, _Step);
                    return lerp(start, ((mainTex.r + mainTex.b + mainTex.g) / 3) * i.color, _Step);
                }
                return lerp(start, mainTex.rgba * i.color, _Step);
            }
            ENDCG
        }
    }
